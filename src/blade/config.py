...
def load_files(blade_root_dir, load_local_config):
    ...
    _blade_config._try_parse_file(os.path.join(os.path.dirname(sys.argv[0]), 'blade.conf'))
    _blade_config._try_parse_file(os.path.expanduser('~/.bladerc'))
    _blade_config._try_parse_file(os.path.join(blade_root_dir, 'BLADE_ROOT'))
    if load_local_config:
        _blade_config._try_parse_file(os.path.join(blade_root_dir, 'BLADE_ROOT.local'))


def get_section(section_name):
    return _blade_config.get_config(section_name)


def get_item(section_name, item_name):
    return _blade_config.get_config(section_name)[item_name]
...
def global_config(append=None, **kwargs):
    ...
    debug_info_levels = _blade_config.get_config('cc_config')['debug_info_levels'].keys()
...
def protoc_plugin(**kwargs):
    ...
    section = _blade_config.get_config('protoc_plugin_config')
