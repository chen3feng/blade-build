...
class TestRunner(binary_runner.BinaryRunner):
    ...
    def _collect_jacoco_classes_and_sources(self):
        """Collect classes and sources to be used as coverage base.
        REturns:
        ...
        for key in self.__command_targets:
            target = self.target_database[key]
            if not target.data.get('jacoco_coverage'):
                continue
            ...
            source_dir = target._get_sources_dir()
            source_dirs.add(source_dir)

        return classes_dirs, source_dirs
    ...
    def _generate_jacoco_coverage_report(self):
        ...
        execfiles = self._collect_jacoco_execfiles()
        if not execfiles:
            return
        ...
        classes_dirs, source_dirs = self._collect_jacoco_classes_and_sources()
